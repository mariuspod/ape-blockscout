...
from ape_blockscout import Blockscout
from ape_blockscout.client import _APIClient
from ape_blockscout.types import BlockscoutResponse
...
@pytest.fixture
def no_api_key():
    key = os.environ.pop("BLOCKSCOUT_API_KEY", None)
    yield
    if key:
        os.environ["BLOCKSCOUT_API_KEY"] = key
...
def response(mocker):
    ...
    return BlockscoutResponse(response, "ethereum", raise_on_exceptions=False)


@pytest.fixture
def mock_backend(mocker):
    session = mocker.MagicMock()
    backend = MockBlockscoutBackend(mocker, session)
    _APIClient.session = session
    return backend


class MockBlockscoutBackend:
    def __init__(self, mocker, session):
        self._mocker = mocker
        self._session = session
        self._expected_base_uri = "https://eth.blockscout.com/api"  # Default
    ...
    def expected_uri_map(
        ...
        return {
            ...
            "gnosis": {
                "mainnet": "https://gnosis.blockscout.com/api",
                "chiado": "https://gnosis-chiado.blockscout.com/api",
            },
            "optimism": {
...
def expected_verification_log(address_to_verify):
    return (
        ...
        f"https://eth.blockscout.io/address/{address_to_verify}#code"
    )
...
def expected_verification_log_with_ctor_args(address_to_verify_with_ctor_args):
    return (
        "Contract verification successful!\n"
        f"https://eth.blockscout.io/address/{address_to_verify_with_ctor_args}#code"
    )
